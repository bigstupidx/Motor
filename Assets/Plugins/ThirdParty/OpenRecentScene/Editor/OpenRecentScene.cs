...
namespace Bosoniq.Tools {
	...
	internal abstract partial class OpenRecentScene : EditorWindow {
		public static readonly System.Version Version = new System.Version("1.1.6");

		public const int SeperatorMenuItemPriority = 1000;
				...
				GUILayout.BeginVertical();
				{
					...
					if (scenePathsMaxIndex < 0) {
						EditorGUILayout.HelpBox(
							"No scenes were found.", MessageType.Info);
					} else {
						...
						if ((this.indexLabelMaxLength != scenePathsMaxIndex)
							|| (this.indexLabelMaxLength <= 0)) {
							this.indexLabelMaxLength = scenePathsMaxIndex;
							indexLabelStyle.fixedWidth = (sceneButtonStyle.CalcSize(new GUIContent(new string('X', ((scenePathsMaxIndex < 10) ? 1 : (1 + Mathf.FloorToInt(Mathf.Log10(scenePathsMaxIndex))))))).x);
						}
							...
							GUILayout.BeginVertical();
							{
								for (int sceneIndex = 0; sceneIndex <= scenePathsMaxIndex; sceneIndex++) {
									...
									if (isActionRequested
										...
										EventModifiers eventModifiers = this.GetCurrentEventModifiers();
										this.delayedAction = () => this.LoadScene(scenePath, eventModifiers);
										this.Repaint();
									}
								}
		...
		protected virtual void AddExtraScenePaths(bool isFullSearchRequested) {
			...
			if ((buildScenes != null)
				...
				// Then scan scenes that are more likely to be included in a build.
				foreach (var scene in buildScenes) {
					if ((scene != null)
						&& (!scene.enabled)) {
						addScenePathIfMissing(scene.path);
		...
		protected virtual void LoadScene(
			...
			if (!string.IsNullOrEmpty(scenePath)) {
				...
				bool isLoadAdditive = ((eventModifiers & (EventModifiers.Control | EventModifiers.Command)) != 0);
				...
				if (isPlayRequest) {
					...
					if ((scenePath != SceneManager.GetActiveScene().name)
						&& string.IsNullOrEmpty(SceneRestoreOnPlayStop)) {
						SceneRestoreOnPlayStop = SceneManager.GetActiveScene().name;
					}
				}
