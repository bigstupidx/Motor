...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        private void _RefreshActive()
        {
            ...
            _ResetVisibleCellViews();
	        if (this.loop) {//Refresh后调用layoutbuild，解决循环列表闪烁,可能存在性能问题,这里只对循环列表处理
				LayoutRebuilder.ForceRebuildLayoutImmediate(this._container);
	        }
        }
        ...
        private void _ScrollRect_OnValueChanged(Vector2 val)
        {
            ...
            // call the handler if it exists // 最后在发送事件
